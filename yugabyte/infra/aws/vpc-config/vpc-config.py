...
def process_terraform():
  ...
  project_cidr = params.get("project_cidr")
  vpc_config = { "config" : json.dumps(generate(project_cidr=project_cidr)) }
  json.dump(vpc_config, sys.stdout)

def generate(
    project_cidr:str="10.98.0.0/16",
    subnets:list[str]=["public", "private"],
    prefix:str='shr',
